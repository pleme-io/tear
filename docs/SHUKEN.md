...
| Graphics needs a new wire endpoint + a daemon-side image store | **CLEARED** (`tear@e7c6ba7`). Sixel arrives through vte's DCS hooks; kitty is APC-framed, which vte cannot surface, so it is lifted out before the parser by a resumable scanner. Payloads are carried **undecoded** on the snapshot — the authority owns transmission, the renderer owns pixels — so no wire endpoint and no daemon-side decoder were needed after all. |

Also landed toward it: the authority itself is sealed (`e343aff`,
`pub(crate)` + a measured `E0624`), and containment exists
(`garasu@1aa5504`).

**All three original blockers are cleared.** What is left is the migration
itself — mechanical, large, and no longer gated on a missing capability.

### The remaining sequence, in order

1. **OSC 8 hyperlinks + a link table.** The one Gate-B item still
   mado-only. Same shape as the combining-mark table: intern, index from
   the cell, carry on the snapshot.
2. **Repoint mado's readers.** `render.rs` (22 sites) and `ux/engine.rs`
   (84) — but per §5-B only the *view* half moves; scroll, selection,
   search and URL detection stay renderer-side because they are functions
   of the view plus operator input, not of the byte stream.
3. **Delete, don't repoint, the seven mode accessors** on mado's `Terminal`
   *and* their `TerminalOps` rows (five have two names each), so every
   missed call site is `E0599` rather than one autocomplete away from the
   wrong answer. `PaneGrid::modes()` is the replacement.
4. **Drop `vte` from mado's manifest** last, once nothing needs it.

### One staging property worth keeping

A half-migrated mado — some readers on the view, some on its own
`Terminal` — is **strictly worse than either endpoint**, because the two
models can then disagree silently, which is the exact defect this decision
exists to remove. Step 2 is therefore one commit, not several. Steps 1 and
3–4 can each stand alone.
